...
However if a specific gene, or set of genes, can be linked to causing cancer in a given cell type, then appropriate siRNA can be synthesised to silence that gene. 
...
The other issue with gene silencing as a treatment method is that siRNA is degraded by enzymes in extracellular space. [cite]
...
Similarly, polymer-based nanoparticles can encapsulate and deliver siRNA in much the same way~\cite{??}. 
...
Cancerous cells generate ATP through glycoysis, which bypasses the normal Krebs cycle and switches off normal mitochondria function. 
...
A potentially exciting drug called dichloroacetate (DCA) is a chemical similar to those involved in the Krebs cycle [cite?]. 
Experiments on cultured cancer cells have shown that DCA can re-establish the Krebs cycle, preventing further cell division and initiating apoptosis. 

Despite its potential, clinical trials have so far failed to provide sufficient evidence that DCA produces a therapeutic response in cancer patients. 
...
The decrease in peak intensity for temperature-treated MOF is due to a loss of crystalline structure from the pore collapse; however peaks are still visible, suggesting that long-range order was not completely destroyed. % Previous work has incuced pore collapse by mechanicmal amorphization, and then peaks completely dissapeared.
...
% Images were taken after ? hours of incubation
...
\begin{figure}[htbp!]
\centering
\includegraphics[width=1.0\textwidth]{mCherry-expression} % Can I get the data for this, and make it all one figure? 
...
As described in Section~\ref{sec:MOF-intro}, most cancers occur when cells bypass the Krebs cycle, such that mitochondria continuously divide without undergoing apoptosis.
This makes cancerous cells difficult to destroy. 
While chemotherapy techniques, such as radiation therapy, can kill the cancer cells, treatment is non-discriminatory and destroys all cells in the area, leading to unwelcome side-effects. 

It has been shown DCA is able to induce apoptosis in cancer cells by restoring mitochondrial function and restarting the Krebs cycle. 
...
In this experiment the MOF UiO-66 was used which, like the NU-1000 used in our previous MOF experiments, has a large pore size and good biocompatibility. 
