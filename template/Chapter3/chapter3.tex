...
We had gathered 3D data utilising optical sectioning on the SIM, as described in Chapter~\ref{chap:LAGSIM}, which now required their specialist biological interpretation. 
...
A virtual light ray begins at a pixel on the screen, and progresses through a 3D array of voxels, as shown in Figure~\ref{fig:raymarch-diagram}~\cite{tuy1984direct}.
...
The primary goal for FPBioimage was to create an easy-to-use web application for visualising volumetric data in a web browser; therefore a requirement was set that it must run without requiring any other plugins or downloads, in order that viewing data was a simple process. 
...
\subsection{The fragment shader calculates the colour of each screen pixel} \label{sec:shader}
...
We should first start at lines 40 and 47, and note that there are two render modes shown: a maximum intensity projection (\texttt{\char`_RenderMode 0}) and the volumetric ray marching mode (\texttt{\char`_RenderMode 1}) discussed in section \ref{sec:volumerendering}.
...
First Person control is shown in Supplementary Video 2 included with this document.
...
In 2000, Ray and Silver produced the Race II Engine, designed for dedicated hardware which could render [512$\times$512$\times$512] volumetric images in real time.
...
Furthermore FPBioimage was designed so that it could be run on any device - Statista statistics that over 40\% of web traffic in August 2018 was from mobile browsers.
...
These programs all support volumetric image data in a wide variety of formats thanks to OME Bioformats{linkert2010metadata}. 
