...
To give readers of a scientific publication a deeper understanding of 3D data, many authors present supplementary fly-through videos showing the data from a variety of perspectives, including from within the volume~\cite{huang2016ultra, legant2016high, fu2016imaging, chhetri2015whole}.
...
If each ray takes \num{512} samples as it passes through the volume, each rendering of the volume requires over 1$\times$\smaller[0.76]{10\textsuperscript{9}} voxel calculations.

When rendering a single volumetric image, or even a video which is first rendered then presented at a later time, this is not an issue: the rays can be computed in parallel across a cluster of computers, and with some patience the images will build up.
...
\subsection[Bookmarking is implemented with a state machine to share perspectives across the world]{Bookmarking is implemented with a state machine to share\\ perspectives across the world}
...
\subsection[Screenshots can be captured at a higher resolution than the display]{Screenshots can be captured at a higher resolution than the\\ display}
...
Finally, for users who want to view volumetric data on their mobile device, I have created dedicated Android and iOS apps, which support virtual reality for truly immersive interaction with the data. % and Oculus?
